...
public class PlanToolTEE {
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
             
            @Override
            public void run() {
                BasicForm1 frame = new BasicForm1("Tasking Expressions");
                frame.createGUI();
            }
        });
    }
...
class BasicPanel1 extends JPanel {
    public BasicPanel1() {
        JButton button = new JButton("Enter New Tasking Expressions");
...
class InputDialog1 extends JDialog {
    public InputDialog1() {
        ...
        setTitle("Input Tasking Expression");
        add(panel);
        setSize(700,700);
         ...
         saveToFileButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    writer.write(verbField.getText() + "\t" + taskerField.getText() + "\t" + 
                            taskeeField.getText() + "\t" + atwhereField.getText() + "\t" + routewhereField.getText()
                    + "\t" + startwhenField.getText() + "\t" + endwhenField.getText() + "\t" + whyField.getText()
                    + "\t" + labelField.getText() + "\t" + modField.getText()
                    );
                    writer.newLine();
                    writer.close();
